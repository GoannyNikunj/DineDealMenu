...
class Category {
  async FilterCategory(req, res) {
    try {
      let Categories = await categoryModel.find({restaurantId:req.params.id}).sort({ _id: -1 });
      if (Categories) {
  ...
  async getAllCategoryFilterByName(req, res) {
    try {
      let Categories = await categoryModel.find({}).sort({ _id: -1 });
  ...
  async postAddCategory(req, res) {
    ...
    if (!restaurantId || !cName || !cDescription || !cStatus || !cImage) {
